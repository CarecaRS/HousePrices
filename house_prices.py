...
xgb_reg = xgb.XGBRegressor(random_state=1,
                           ...
                           n_jobs=12,
                           reg_alpha=0.01,
                           device='cuda',  # gpu, cpu, cuda
                           enable_categorical=False,
                           eval_metric='rmsle',
                           early_stopping_rounds=50)

# Model fitting
xgb_reg.fit(train_x_ohe, train_y_ohe,
            eval_set = [(train_x_ohe, train_y_ohe), (test_x_ohe, test_y_ohe)],
            verbose = 100)

# Model cross-validation (assessing the model's robustness)
cv_scores_xgb = cross_val_score(xgb_reg, train_x_ohe, y = train_y_ohe,
                                n_jobs = 12,
                                verbose = 0,
                                params = {'eval_set':[(train_x_ohe, train_y_ohe), (test_x_ohe, test_y_ohe)], 'verbose':0}, 
                                error_score = 'raise')

# Prediction analysis
ypred_xgb = xgb_reg.predict(test_x_ohe)
score_xgb_r2 = r2_score(test_y_ohe, ypred_xgb)
rmsle_xgb = root_mean_squared_log_error(test_y_ohe, ypred_xgb)

print(f'\nLocal RMSLE: {rmsle_xgb:.6f}')
print(f'Coefficient of Determination (R2): {score_xgb_r2:.6f}')
print(f'Cross-validation score: {cv_scores_xgb.mean():.6f}')

#### SUBMISSION
# Predicts the target on test data
ypred_xgb_final = xgb_reg.predict(ohe_test)
# Generates the name to save file
print(f'Prediction in use has record #{nome_modelo}.')
print('Compiling filename to submission...')
ext_final = ('.csv')
save_final = ('XGBRegressor_OHE_'+ str(round(rmsle_xgb, 5))+'_'+(nome_modelo + ext_final))
...
submissao['SalePrice'] = ypred_xgb_final
...
# Model cross-validation (assessing the model's robustness)
cv_scores_xgb = cross_val_score(xgb_reg, train_x, y = train_y,
                                n_jobs = 12,
                                verbose = 0,
                                params = {'eval_set':[(train_x, train_y), (test_x, test_y)], 'verbose':0}, 
                                error_score = 'raise')

# Prediction analysis
ypred_xgb = xgb_reg.predict(test_x)
score_xgb_r2 = r2_score(test_y, ypred_xgb)
rmsle_xgb = root_mean_squared_log_error(ypred_xgb, test_y)
print(f'\nLocal RMSLE: {rmsle_xgb:.6f}')
print(f'Coefficient of Determination (R2): {score_xgb_r2:.6f}')
print(f'Cross-validation score: {cv_scores_xgb.mean():.6f}')
...
#### SUBMISSION
# Predicts the target on test data
ypred_xgb_final = xgb_reg.predict(noohe_test)
# Generates the name to save file
print(f'Prediction in use has record #{nome_modelo}.')
print('Compiling filename to submission...')
ext_final = ('.csv')
save_final = ('XGBRegressor_'+ str(round(rmsle_xgb, 5))+'_'+(nome_modelo + ext_final))
print('Generating submission file named ' + save_final)
# Reads submission file, records the obtained data and rewrites the file with it
submissao = pd.read_csv('sample_submission.csv')
submissao['SalePrice'] = ypred_xgb_final
...
# We need to code a custom metric for CatBoost, since it doesn't have
# RMSLE
...
cat_reg = CatBoostRegressor(iterations=2000,
                        ...
                        random_seed=1,
                        use_best_model=True,
                        verbose=True,  # may be defined by an integer also
                        one_hot_max_size=255,  # 255 if training in GPU, 2 in CPU
                        eval_metric=RMSLE_val(),
                        boosting_type='Ordered',  # Ordered (better for small datasets)/Plain
                        task_type='GPU',  # GPU/CPU, but GPU can't run with custom eval_metric/loss_function
                        reg_lambda=1.3,
                        early_stopping_rounds=50,
                        grow_policy='SymmetricTree',  # SymmetricTree (def), Depthwise, Lossguide
                        score_function='Cosine')  # Cosine, L2, NewtonL2, NewtonCosine

# Model fitting
cat_reg.fit(train_x_ohe, train_y_ohe,
            eval_set = (test_x_ohe, test_y_ohe),
            verbose = 200)

# Model cross-validation info
# CatBoost has a proper function to perform cross-validation, which takes
# a lot less parameters then the regular model. Therefore I'm choosing
# not to perform CV on it. If you want to try it out, check
# https://catboost.ai/docs/en/concepts/python-reference_cv

# Prediction analysis
ypred_cat = cat_reg.predict(test_x_ohe)
score_cat_r2 = r2_score(test_y_ohe, ypred_cat)
rmsle_cat = root_mean_squared_log_error(ypred_cat, test_y_ohe)
print(f'\nLocal RMSLE: {rmsle_cat:.6f}')
print(f'Coefficient of Determination (R2): {score_cat_r2:.6f}')

#### SUBMISSION
# Predicts the target on test data
ypred_cat_final = cat_reg.predict(ohe_test)
# Generates the name to save file
print(f'Prediction in use has record #{nome_modelo}.')
print('Compiling filename to submission...')
ext_final = ('.csv')
save_final = ('CatBoostRegressor_OHE_'+ str(round(rmsle_cat, 5))+'_'+(nome_modelo + ext_final))
print('Generating submission file named ' + save_final)
# Reads submission file, records the obtained data and rewrites the file with it
submissao = pd.read_csv('sample_submission.csv')
submissao['SalePrice'] = ypred_cat_final
submissao.to_csv('./resultados/'+save_final, index=False)
print("\nSuccess. File is in directory ('./resultados/').")


#### Non-OHE Model 
# CatBoost needs a list with the categorical features, so we'll do this first
# and then perform the training like before
cat_features = train_x.dtypes[train_x.dtypes == 'category'].index.values
...
# Prediction analysis
ypred_cat = cat_reg.predict(test_x)
score_cat_r2 = r2_score(test_y, ypred_cat)
rmsle_cat = root_mean_squared_log_error(ypred_cat, test_y)
print(f'\nLocal RMSLE: {rmsle_cat:.6f}')
print(f'Coefficient of Determination (R2): {score_cat_r2:.6f}')

#### SUBMISSION
# Predicts the target on test data
ypred_cat_final = cat_reg.predict(noohe_test)
# Generates the name to save file
print(f'Prediction in use has record #{nome_modelo}.')
print('Compiling filename to submission...')
ext_final = ('.csv')
save_final = ('CatBoostRegressor_'+ str(round(rmsle_cat, 5))+'_'+(nome_modelo + ext_final))
print('Generating submission file named ' + save_final)
# Reads submission file, records the obtained data and rewrites the file with it
submissao = pd.read_csv('sample_submission.csv')
submissao['SalePrice'] = ypred_cat_final
...
#### OHE Model
nome_modelo = datetime.now().strftime('%Y%m%d-%H%M')  # I like to record the inicial time for the model's name at the end
lgbm_reg = LGBMRegressor(boosting_type='gbdt',  # gbdt, dart, rf
                         objective='root_mean_squared_error',
                         num_leaves=16, # default 31
                         max_depth=5,  # default -1 (no limit)
                         learning_rate=0.01,
                         n_estimators=1000,
...
# Model fitting
lgbm_reg.fit(train_x_ohe, train_y_ohe,
            eval_set = [(train_x_ohe, train_y_ohe), (test_x_ohe, test_y_ohe)])

# Model cross-validation (assessing the model's robustness)
#cv_scores_lgbm = cross_val_score(lgbm_reg, train_x_ohe, y = train_y_ohe,
#                                n_jobs = 12,
#                                verbose = 0,
#                                params = {'eval_set':[(train_x_ohe, train_y_ohe), (test_x_ohe, test_y_ohe)]}, 
#                                error_score = 'raise')

# Prediction analysis
ypred_lgbm = lgbm_reg.predict(test_x_ohe)
score_lgbm_r2 = r2_score(test_y_ohe, ypred_lgbm)
rmsle_lgbm = root_mean_squared_log_error(ypred_lgbm, test_y_ohe)
print(f'\nLocal RMSLE: {rmsle_lgbm:.6f}')
print(f'Coefficient of Determination (R2): {score_lgbm_r2:.6f}')
#print(f'Cross-validation score: {cv_scores_lgbm.mean():.6f}')

#### SUBMISSION
# Predicts the target on test data
ypred_lgbm_final = lgbm_reg.predict(ohe_test)
# Generates the name to save file
print(f'Prediction in use has record #{nome_modelo}.')
print('Compiling filename to submission...')
ext_final = ('.csv')
save_final = ('LGBMRegressor_OHE_'+ str(round(rmsle_lgbm, 5))+'_'+(nome_modelo + ext_final))
print('Generating submission file named ' + save_final)
# Reads submission file, records the obtained data and rewrites the file with it
submissao = pd.read_csv('sample_submission.csv')
submissao['SalePrice'] = ypred_lgbm_final
submissao.to_csv('./resultados/'+save_final, index=False)
print("\nSuccess. File is in directory ('./resultados/').")
...
# Model cross-validation (assessing the model's robustness)
#cv_scores_lgbm = cross_val_score(lgbm_reg, train_x, y = train_y,
#                                n_jobs = 12,
#                                verbose = 0,
#                                params = {'eval_set':[(train_x, train_y), (test_x, test_y)], 'verbose':0}, 
#                                error_score = 'raise')
...
# Prediction analysis
ypred_lgbm = lgbm_reg.predict(test_x)
score_lgbm_r2 = r2_score(test_y, ypred_lgbm)
rmsle_lgbm = root_mean_squared_log_error(ypred_lgbm, test_y)
print(f'\nLocal RMSLE: {rmsle_lgbm:.6f}')
print(f'Coefficient of Determination (R2): {score_lgbm_r2:.6f}')
#print(f'Cross-validation score: {cv_scores_lgbm.mean():.6f}')
...
#### SUBMISSION
# Predicts the target on test data
ypred_lgbm_final = lgbm_reg.predict(noohe_test)
# Generates the name to save file
print(f'Prediction in use has record #{nome_modelo}.')
print('Compiling filename to submission...')
ext_final = ('.csv')
save_final = ('LGBMRegressor_'+ str(round(rmsle_lgbm, 5))+'_'+(nome_modelo + ext_final))
print('Generating submission file named ' + save_final)
# Reads submission file, records the obtained data and rewrites the file with it
submissao = pd.read_csv('sample_submission.csv')
submissao['SalePrice'] = ypred_lgbm_final
...
sklrf_reg = RandomForestRegressor(n_estimators=2000,
                                  criterion='squared_error', 
                                  max_depth=12,
                                  bootstrap=True,  # default False
                                  oob_score=True,  # only possible when bootstrap=True
                                  n_jobs=12,
                                  random_state=1,
                                  verbose=1,
                                  max_samples=0.95)  # use when bootstrap=True

# Model fitting
sklrf_reg.fit(train_x_ohe, train_y_ohe)

# Model cross-validation (assessing the model's robustness)
cv_scores_sklrf = cross_val_score(sklrf_reg, train_x_ohe, y = train_y_ohe,
                                  n_jobs = 12,
                                  verbose = 0,
                                  error_score = 'raise')

# Prediction analysis
ypred_sklrf = sklrf_reg.predict(test_x_ohe)
score_sklrf_r2 = r2_score(test_y_ohe, ypred_sklrf)
rmsle_sklrf = root_mean_squared_log_error(ypred_sklrf, test_y_ohe)
print(f'\nLocal RMSLE: {rmsle_sklrf:.6f}')
print(f'Coefficient of Determination (R2): {score_sklrf_r2:.6f}')
print(f'Cross-validation score: {cv_scores_sklrf.mean():.6f}')

#### SUBMISSION
# Predicts the target on test data
ypred_sklrf_final = sklrf_reg.predict(ohe_test)
# Generates the name to save file
print(f'Prediction in use has record #{nome_modelo}.')
print('Compiling filename to submission...')
ext_final = ('.csv')
save_final = ('SklearnRandomForest_OHE_'+ str(round(rmsle_sklrf, 5))+'_'+(nome_modelo + ext_final))
print('Generating submission file named ' + save_final)
# Reads submission file, records the obtained data and rewrites the file with it
submissao = pd.read_csv('sample_submission.csv')
submissao['SalePrice'] = ypred_sklrf_final
submissao.to_csv('./resultados/'+save_final, index=False)
print("\nSuccess. File is in directory ('./resultados/').")
...
#### Non-OHE Model
# Sklearn's regressor RandomForest does NOT deal with categorical features
